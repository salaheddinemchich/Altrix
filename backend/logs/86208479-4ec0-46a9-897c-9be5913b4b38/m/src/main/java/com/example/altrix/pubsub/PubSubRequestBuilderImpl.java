...
/**
 * Generic splitter — chops a collection of {@code T} into a list of Kafka requests
    ...
    for (T item : items) {
      ...
      if (size > constraints.getMaxItemBytes()) {
        throw new RuntimeException(
            "Item exceeds max item size (" + size + " > " + constraints.getMaxItemBytes() + " bytes): " + payloadAsString.apply(item));
      }
      boolean wouldOverflow =
          current.size() + 1 > constraints.getMaxItemsPerRequest()
