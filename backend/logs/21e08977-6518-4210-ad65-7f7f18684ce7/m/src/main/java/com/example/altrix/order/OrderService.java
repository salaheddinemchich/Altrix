...
public class OrderService {
    ...
    private void publishOrderCreated(Order order) {
        ...
        headers.put("orderId", order.getId());
        headers.put("customerEmail", order.getCustomerEmail());
        headers.put("total", order.getTotal().toPlainString());
        headers.put("eventType", "order.created");

        String payload = "{\"orderId\":\"" + order.getId() + "\",\"product\":\"" + order.getProduct() + "\",\"quantity\":" + order.getQuantity() + ",\"total\":" + order.getTotal() + "}";
        ProducerRecord<String, String> record = new ProducerRecord<>(PubsubConfig.ORDERS_CREATED, order.getId(), payload, headers);

        try {
            pubsubService.publish(record);
