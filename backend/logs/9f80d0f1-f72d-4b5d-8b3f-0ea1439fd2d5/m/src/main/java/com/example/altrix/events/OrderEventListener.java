...
public class OrderEventListener {
    ...
    @PostConstruct
    void onStartup() {
        log.info("Initializing Kafka consumer for orders.created");
        ...
        consumer.subscribe(List.of(PubsubConfig.ORDERS_CREATED));
    }
    ...
    public void pollForOrders() {
        ...
        for (ConsumerRecord<String, String> record : records) {
            try {
                String orderId = record.headers().last("orderId").value();
                String totalStr = record.headers().last("total").value() != null ? 
                                 record.headers().last("total").value() : "0";
                ...
                ackIds.add(record.offset() + ""); // Note: Kafka doesn't use ack IDs like Pub/Sub
            } catch (Exception e) {
                log.error("Failed to process received order message offset={}", record.offset(), e);
            }
        }
        // Kafka auto-commits by default, but if manual commits are needed:
        // consumer.commitSync(); // or consumer.commitAsync() for async
        consumer.poll(0); // Ensure consumer is alive between polls
    }
}